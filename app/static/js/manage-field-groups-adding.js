...
nFieldGroupsContainers.forEach((nFieldGroupsContainer) => {
  ...
  btnRemoveFirstFieldGroup.addEventListener("click", () => {
    removeFieldGroup(getFirstFieldGroup(fieldGroups), fieldGroups);
  });

  const btnAddFieldGroup = nFieldGroupsContainer.querySelector(
    ".btn-add-field-group"
  );
  btnAddFieldGroup.addEventListener("click", () => {
...
function addFieldGroup(fieldGroups) {
  ...
  newFieldGroup.querySelectorAll("input").forEach((input) => {
    input.value = "";
  });

  const btnRemoveDefinitionField = newFieldGroup.querySelector(
    `button.btn-remove-field-group`
  );
  btnRemoveDefinitionField.disabled = false;
  getFirstFieldGroup(fieldGroups).querySelector(
    "button.btn-remove-field-group"
  ).disabled = false;

  btnRemoveDefinitionField.addEventListener("click", () => {
...
function getFirstFieldGroup(fieldGroups) {
  fieldGroups.querySelectorAll(".field-group").item(0);
}
